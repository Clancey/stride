...
class _AppsScreenState extends State<AppsScreen> with SpikeLog {
  ...
  Future<void> _load() => guard('list apps', () async {
        ...
        for (final a in likely) {
          logLine('  ${mediaLikelihood(a).toString().padLeft(3)}  '
              '${a['label']}  (${a['package']})');
        }
      });
  ...
  Widget build(BuildContext context) {
    return SpikeScaffold(
      title: 'S4 — Apps',
      question: 'Which apps install and launch here? Netflix on a likely Widevine-L3, '
            ...
            return ListTile(
              ...
              trailing: IconButton(
                ...
                onPressed: () => guard('launch ${app['package']}', () async {
                  final ok = await SpikeBridge.launchApp(app['package'] as String);
