...
class _OverlayScreenState extends State<OverlayScreen> with SpikeLog {
  ...
  Widget build(BuildContext context) {
    ...
    return SpikeScaffold(
      title: 'S3 — Overlay',
      question:
          'Does the overlay survive real apps, and how much touch do the edge strips steal?',
      log: log,
      actions: [
        FilledButton.icon(
          onPressed: () => guard('start overlay', () async {
            ...
            logLine(
              ok
                  ...
                  : 'FAIL Could not start - check permission.',
            );
            await _refreshQuiet();
          }),
        ...
        OutlinedButton.icon(
          onPressed: () => guard('reset counters', () async {
            ...
            logLine(
              'Interference counters reset. Attribute the next run to one foreground app.',
            );
            await _refreshQuiet();
          }),
