...
**The incline column can be given wider steps.** Settings → Advanced → Incline quick picks. It is off by default and the column you have now does not change unless you turn it on. Turning it on spaces the buttons 5% apart climbing and 3% apart declining — not one step for both, because a treadmill that declines at all rarely goes past -6%, so a single wide step would leave you one decline button. The machine's own highest and lowest inclines are always on the column whichever you pick. On a console reporting -6% to 40% that is eleven buttons instead of forty. This only applies where Stride builds the column itself; on iFit the console publishes its own quick picks and those are used as they are.
